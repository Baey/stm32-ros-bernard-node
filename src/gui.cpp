...
BernardGUI::BernardGUI(Adafruit_ST7735* tft, HardwareTimer* screenRefreshTimer,
    ...
    : tft(tft), screenRefreshTimer(screenRefreshTimer), currentRobotStatus(robotStatus), previousRobotStatus(), logIndex(0), previousLogIndex(0), nextScreen(screen) {
...
void BernardGUI::drawStatusScreen() {
    ...
    if (previousRobotStatus != currentRobotStatus || initialStatusDraw) {
        if ((previousRobotStatus->IMUStatus != currentRobotStatus->IMUStatus) ||
            ...
            switch (this->currentRobotStatus->ROSStatus || initialStatusDraw) {
                case WAITING_AGENT:
        ...
        if (previousRobotStatus->euler != currentRobotStatus->euler) {
            ...
            tft->fillRect(STATUS_VALUE_X, TOP_MARGIN + LEFT_MARGIN * 3, tft->width() - LEFT_MARGIN, LINE_HEIGHT * 3, ST7735_BLACK);
    ...
    if (previousRobotStatus->temperature != currentRobotStatus->temperature || initialStatusDraw) {
        // Clear the previous temperature area
        tft->fillRect(STATUS_VALUE_X, TOP_MARGIN + LEFT_MARGIN * 6, tft->width() - LEFT_MARGIN, LINE_HEIGHT, ST7735_BLACK);

        // Draw the IMU temperature
        tft->setTextColor(ST77XX_WHITE);
        tft->setCursor(LEFT_MARGIN, TOP_MARGIN + LINE_HEIGHT * 6);
        tft->print("TEMP: ");
        tft->setCursor(STATUS_VALUE_X, TOP_MARGIN + LINE_HEIGHT * 6);
        tft->setTextColor(ST77XX_CYAN);
        tft->print(this->currentRobotStatus->temperature);
        tft->print(" C");
    }

    if (previousRobotStatus->ipAddress != currentRobotStatus->ipAddress || initialStatusDraw) {
        // Clear the previous IP address area
        tft->fillRect(STATUS_VALUE_X, TOP_MARGIN + LEFT_MARGIN * 7, tft->width() - LEFT_MARGIN, LINE_HEIGHT, ST7735_BLACK);

        // Draw the IP address
        tft->setTextColor(ST77XX_WHITE);
        tft->setCursor(LEFT_MARGIN, TOP_MARGIN + LINE_HEIGHT * 7);
        tft->print("IP: ");
        tft->setCursor(STATUS_VALUE_X, TOP_MARGIN + LINE_HEIGHT * 7);
        tft->setTextColor(ST77XX_CYAN);
        tft->print(this->currentRobotStatus->ipAddress.c_str());
    }

    initialStatusDraw = false;
